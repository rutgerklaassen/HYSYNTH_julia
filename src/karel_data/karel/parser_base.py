...
class Parser(object):
    ...
    def run(self, code, with_timeout=False, **kwargs):
        ...
        out = fn()
        return out
    def run_with_trace(self, code, include_initial=True, **kwargs):
        """
        Run code and return (out, trace_states).
        'trace_states' is a list of state tensors after each *action*,
        and optionally includes the initial state when include_initial=True.
        """
        # start a fresh trace on the underlying Karel instance
        self.karel.begin_trace(include_initial=include_initial)
        out = self.run(code, **kwargs)
        trace = self.karel.end_trace()
        return out, trace
