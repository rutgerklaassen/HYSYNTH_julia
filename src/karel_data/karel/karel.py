...
def hero_action(func):
    def fn(*args, **kwargs):
        self = args[0]
        out = func(self)
        # append state snapshot after ACTION
        if getattr(self, "_trace_enabled", False):
            # store a *copy* to avoid later mutation issues
            self._trace.append(self.state.copy())
        if self.debug:
...
# Conditions should NOT record traces:
def condition_action(func):
    def fn(*args, **kwargs):
        self = args[0]
        out = func(self)
        if self.debug:
...
# Use the non-tracing decorator for world conditions
world_condition = condition_action
...
class Karel(object):
    ...
    def begin_trace(self, include_initial=True):
        """Enable tracing and reset the buffer. Optionally include the initial state."""
    ...
    def end_trace(self):
        """Disable tracing and return the buffered states."""
        self._trace_enabled = False
